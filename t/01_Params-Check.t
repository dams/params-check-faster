...
### use && import ###
BEGIN {
    use_ok( 'Params::Check::Faster' );
    Params::Check::Faster->import(qw|check last_error allow|);
}    

### verbose is good for debugging ###
$Params::Check::Faster::VERBOSE = $Params::Check::Faster::VERBOSE = $ARGV[0] ? 1 : 0;
...
{   
    ...
    {   local $Params::Check::Faster::STRIP_LEADING_DASHES = 1;
        my $try  = { -foo => 2 };
        my $get  = { foo  => 2 };
...
{   my $tmpl = { Foo => { default => 1 } };
    ...
    for (1,0) {
        local $Params::Check::Faster::PRESERVE_CASE = $_;
...
{   
    ...
    ### allow unknown ###
    {
        local   $Params::Check::Faster::ALLOW_UNKNOWN = 1;
        my $rv = check( {}, { foo => 42 } );        
...
{   my $foo;
    ...
    for( 1, 0 ) {
        local   $Params::Check::Faster::NO_DUPLICATES = $_;
...
{   my @list = (
    ...
    for my $aref (@list) {
        ...
        my $tmpl = { foo => $aref->[0] };
        local   $Params::Check::Faster::STRICT_TYPE = $aref->[1];
...
{   my @list = (
    ...
    for my $aref (@list) {
        ...
        my $tmpl = { foo => $aref->[0] };
        local   $Params::Check::Faster::ONLY_ALLOW_DEFINED = $aref->[1];
...
### $Params::Check::Faster::CALLER_DEPTH test
{
    sub wrapper { check  ( @_ ) };
    sub inner   { wrapper( @_ ) };
    sub outer   { inner  ( @_ ) };
    ...
    local $Params::Check::Faster::CALLER_DEPTH = 1;
    outer( { dummy => { required => 1 }}, {} );
